...
class AidProgram(models.Model):
    # Status choices for the aid program (whether applications are open or closed).
    STATUS_CHOICES = [
        ('OPEN', 'Open'),
        ('CLOSE', 'Close'),
    ]
    
    # Defines whether the aid program is for local or international students.
    LOCALITY_CHOICES = [
        ('LOCAL', 'Local'),
        ('INTERNATIONAL', 'International'),
    ]
    
    # Specifies the education level the aid program is for.
    LEVEL_CHOICES = [
        ('UNDERGRADUATE', 'Undergraduate'),
        ('POSTGRADUATE', 'Postgraduate'),
    ]

    name = models.CharField(max_length=255)  # Name of the aid program.
    owner = models.CharField(max_length=255)  # Entity or person managing the aid program.
    aid_type = models.CharField(max_length=100)  # Type of aid (scholarship, grant, loan, etc.).
    level = models.CharField(max_length=20, choices=LEVEL_CHOICES)  # Education level eligible for aid.
    locality = models.CharField(max_length=20, choices=LOCALITY_CHOICES)  # Local or international aid.
    status = models.CharField(max_length=10, choices=STATUS_CHOICES, default='OPEN')  # Current status of the aid program.
    eligibility = models.TextField(blank=True, null=True)  # Eligibility criteria (optional).
    closing_date = models.DateField()  # Deadline for applications.
    ...
    def __str__(self):
        return self.name  # Returns the name of the aid program when printed.
